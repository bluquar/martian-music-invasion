...
public class BonusLevelManager : MonoBehaviour {
    ...
    {
        Camera.allCameras[0].backgroundColor = Color.black;
        yield return Transition.Translate(this.transform, this.HidingTransform, this.ShowTime);
    ...
    private void ChooseSpaceship()
    {
        ...
        SelectedSpaceship = SeeBoxFront.transform.FindChild(name).gameObject;
    }
    ...
    private string ChooseMusic(out AudioClip clip)
    {
        ...
        SelectedMeasure = HearBoxFront.transform.FindChild(name).gameObject;
        return name;
    }
    ...
    private IEnumerator CycleOptions(List<GameObject> options, GameObject choice)
    {
        ...
        foreach (GameObject other in options)
        {
            if (other != choice)
            {
                StartCoroutine(Transition.FadeOut(other, fadeOutTime, false));
            }
        }
    ...
    private IEnumerator MoveSelectionObject(GameObject obj, GameObject dest, GameObject oldContainer)
    {
        ...
        StartCoroutine(Transition.FadeOut(oldContainer, 0.8f, false));
    ...
    private IEnumerator ShowSelection()
    {
        ...
        yield return Transition.FadeIn(SelectionPane, 1f, true);
